...
void solver(bool& solved, char**& maze, int& rows, int& cols, int xpos, int ypos) {
	...
	if(xpos < rows) {
		//cout << "Up is: " << maze[xpos - 1][ypos] << endl;
		if(maze[xpos - 1][ypos] != 'X' && maze[xpos - 1][ypos] != '!') solver(solved, maze, rows, cols, (xpos - 1), ypos);
	}
	if(ypos < cols) {
		//cout << "Right is: " << maze[xpos][ypos + 1] << endl;
		if(maze[xpos][ypos + 1] != 'X' && maze[xpos][ypos + 1] != '!') solver(solved, maze, rows, cols, xpos, (ypos + 1));
	}
	if(ypos > 0) {
		//cout << "Left is: " << maze[xpos][ypos - 1] << endl;
		if(maze[xpos][ypos - 1] != 'X' && maze[xpos][ypos - 1] != '!') solver(solved, maze, rows, cols, xpos, (ypos - 1));
	}
	if(xpos > 0) {
		//cout << "Down  is: " << maze[xpos + 1][ypos] << endl;
		if(maze[xpos + 1][ypos] != 'X' && maze[xpos + 1][ypos] != '!') solver(solved, maze, rows, cols, (xpos + 1), ypos);
	}
